...
export function pack(rects: Rect[], binWidth: number, binHeight: number): PackResult {
  ...
  return results.reduce((best, r) => {
    ...
    if (r.packed.length === best.packed.length) {
      const bbox = (res: PackResult): number =>
        res.packed.reduce(
          (max, p) => Math.max(max, (p.y + p.height) * binWidth + (p.x + p.width)),
          0,
        );
...
function splitFreeRects(
  ...
  let index = freeRects.length;

  while (index--) {
    const fr = freeRects[index];

    if (!overlaps(fr, placement.x, placement.y, w, h)) {
      continue;
    }
...
function pruneFreeRects(freeRects: FreeRect[]): void {
  ...
  while (index--) {
    let otherIndex = freeRects.length;
    ...
    while (otherIndex--) {
      ...
      if (contains(freeRects[otherIndex], freeRects[index])) {
        freeRects.splice(index, 1);
        break;
      }
    }
