  ...
  for (const codePoint of req.codePoints) {
    ...
    if (!glyph || glyph.index === 0) {
      ...
      const xadvance = Math.round(
        (glyph?.advanceWidth ?? spaceGlyph?.advanceWidth ?? req.fontSize / 2) * scale,
      );
    ...
    const ctx = canvas.getContext('2d')!; // OffscreenCanvas always returns a 2D context
    ...
    for (let index = 0; index < pixels.length; index++) {
      pixels[index] = imageData.data[index * 4 + 3];
    }
