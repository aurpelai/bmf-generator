...
describe('makeBlankGlyph', () => {
  it('returns a glyph with the requested code point and project id', () => {
    const glyph = makeBlankGlyph('project-1', 0x41, 8, 12);

    expect(glyph.codePoint).toBe(0x41);
    expect(glyph.projectId).toBe('project-1');
  });
  ...
  it('zero-initialises the pixel buffer', () => {
    const glyph = makeBlankGlyph('project-1', 0x41, 4, 4);
...
describe('initializeGlyphs', () => {
  it('produces one glyph per code point, preserving order', () => {
    ...
    const glyphs = initializeGlyphs('project-1', codePoints, 8, 12);
  ...
  it('returns an empty array when no code points are given', () => {
    expect(initializeGlyphs('project-1', [], 8, 12)).toEqual([]);
  });

  it('sizes every glyph buffer identically', () => {
    const glyphs = initializeGlyphs('project-1', [0x41, 0x42], 6, 10);
