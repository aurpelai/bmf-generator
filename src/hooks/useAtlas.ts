...
export function useAtlas() {
  ...
  function getWorker(): Worker {
    if (!workerRef.current) {
      ...
      workerRef.current.onmessage = (e: MessageEvent<AtlasWorkerResponse>) => {
        const { id, placements, atlasImageData, atlasWidth, atlasHeight, efficiency, unpacked, error } = e.data
        const pending = pendingRef.current.get(id)
        if (!pending) return
        pendingRef.current.delete(id)
        if (error) pending.reject(new Error(error))
        else if (placements && atlasImageData && atlasWidth && atlasHeight && efficiency !== undefined && unpacked)
          pending.resolve({ placements, atlasImageData, atlasWidth, atlasHeight, efficiency, unpacked })
      }
    }
  ...
  const packAtlas = useCallback(
    (glyphs: Glyph[], padding: number): Promise<AtlasResult> => {
