...
export function useRasterize() {
  ...
  const pendingRef = useRef<Map<string, { resolve: (r: RasterizeResult) => void; reject: (e: Error) => void }>>(new Map())

  function getWorker(): Worker {
    if (!workerRef.current) {
      workerRef.current = new Worker(new URL('../workers/rasterize.worker.ts', import.meta.url), { type: 'module' })
